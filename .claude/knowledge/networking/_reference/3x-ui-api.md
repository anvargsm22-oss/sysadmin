...
## 3. Helper-функция для скиллов — код, а не копия здесь

Ходить в API руками скиллам не нужно: есть общий хелпер. **Источник правды — сам файл
`scripts/lib-api/3xui.sh` в корне репозитория** (927 строк, ~20 функций `api_*`).
Интерфейс здесь не переписывается: копия неизбежно разъезжается с кодом, что и произошло —
до 05.08.2026 в этом разделе стояли неверный путь (`scripts/_lib-api.sh` внутри скилла),
неверное число потребителей и способ подключения через `$0`, который в inline-блоке даёт
имя оболочки, а не путь скрипта.

⚠️ Последнее — **не покрыто** линтером `scripts/check-shell-portability.sh`: правила на `$0`
у него нет, старая версия этого раздела проходила замок насквозь (проверено прогоном линтера
по версии файла из коммита `06c106b`). Родственный класс закрыт ADR-0028, но конкретно эта
форма живёт вне его — значит, ловится только чтением.

**Как подключают его реальные скрипты** — считают корень репозитория от собственного пути
(`BASH_SOURCE`), поднимаются на четыре уровня и сорсят `scripts/lib-api/3xui.sh`. Идиому
смотреть в любом из скриптов-потребителей, например
`.claude/skills/configure-vpn-routing/scripts/create-vless-inbound.sh`, строки 31-36.

⚠️ Дословно сюда она не выписана намеренно: этот блок — **не рецепт для вставки в оболочку**.
Приём с `BASH_SOURCE` законен внутри файла с шебангом bash и неверен в inline-блоке, который
агент вставляет в оболочку оператора (там `BASH_SOURCE` даст имя оболочки). Линтер
`scripts/check-shell-portability.sh` не различает документирование и рецепт — и правильно
делает (ADR-0028).

**Кто им пользуется** (замер 05.08.2026, файлы `*.sh`, сорсящие хелпер) — пять скиллов:
`/setup-vpn-panel`, `/configure-vpn-routing`, `/generate-client-config`,
`/setup-server-proxy`, `/finalize-vpn-routing`.

Точные имена, аргументы и коды возврата функций читать в самом файле. Хелпер абстрагирует:
