...
echo "[1б] crontab -l — чтение, не изменение (ложное срабатывание 2026-09-25)"
mk "$TMP/cron-ro1.jsonl" "Bash:ssh prod-host 'crontab -l'"
check pass "crontab -l через ssh" "$TMP/cron-ro1.jsonl"
mk "$TMP/cron-ro2.jsonl" "Bash:sudo crontab -u root -l | grep backup"
check pass "crontab -u root -l" "$TMP/cron-ro2.jsonl"
mk "$TMP/cron-ch1.jsonl" "Bash:ssh prod-host 'crontab -l | sed s/x/y/ | crontab -'"
check block "crontab - (запись из stdin)" "$TMP/cron-ch1.jsonl"
mk "$TMP/cron-ch2.jsonl" "Bash:ssh prod-host 'crontab /tmp/new.cron'"
check block "crontab файл" "$TMP/cron-ch2.jsonl"
mk "$TMP/cron-ch3.jsonl" "Bash:ssh prod-host 'crontab -u root -r'"
check block "crontab -u root -r" "$TMP/cron-ch3.jsonl"
